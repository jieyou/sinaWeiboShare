...
<pre>
sinaWeiboShare.action(optionsOrUrl) // 直接打开分享到微博的页面，参数形式为Object或String。为Object时，具体字段见下“options字段”；为String时，即直接将 @.getUrl@ 方法获得的url传递进来。这个方法使用的是 @window.open@ 方式打开微博分享页面
sinaWeiboShare.getUrl(options) // 将指定的options生成对应微博分享的页面链接，获取这个链接后，可以自行通过 @<a href=""></a>@ @window.open@ 或 @location.href = @ 等或调用 @.action@ 方法打开微博分享页面
</pre>
